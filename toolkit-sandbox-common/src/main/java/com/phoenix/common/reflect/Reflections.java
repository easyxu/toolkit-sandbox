...
public class Reflections {
		...
		try {
			...
			if (args != null && args.length > 0) {
				message += " with parameters: ("
						+ StringUtil.toClassNameString(", ", args) + ')';
			}
	...
	private static String toString(Method method) {
		return StringUtil.unqualify(method.getDeclaringClass().getName()) + '.'
				+ method.getName() + '('
				+ StringUtil.toString(", ", (Object)method.getParameterTypes()) + ')';
	}

	private static String toString(Field field) {
		return StringUtil.unqualify(field.getDeclaringClass().getName()) + '.'
				+ field.getName();
	}
