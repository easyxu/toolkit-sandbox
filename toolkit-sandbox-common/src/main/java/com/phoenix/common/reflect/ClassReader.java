...
public class ClassReader extends ByteArrayInputStream
{
	...
	{
		InputStream fin = c.getResourceAsStream('/'
	...
	protected static String getSignature(Member method, Class[] paramTypes)
	{
		...
		StringBuffer b = new StringBuffer((method instanceof Method) ? method
				.getName() : "<init>");
		b.append('(');
		...
		try
		{
			...
			if (m == null)
			{
				...
				if (nt.name.equals("<init>"))
				{
					...
					{
						String sig = getSignature(ctors[i], ctors[i]
								.getParameterTypes());
				...
				else
				{
					...
					for (int i = 0; i < methods.length; i++)
					{
						String sig = getSignature(methods[i], methods[i]
								.getParameterTypes());
						if (sig.equals(signature))
						{
