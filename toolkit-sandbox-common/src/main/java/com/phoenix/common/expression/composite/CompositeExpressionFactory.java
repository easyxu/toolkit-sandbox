...
import com.phoenix.common.expression.jexl.JexlExpressionFactory;

/**
 * 创建<code>CompositeExpression</code>的工厂。
 *
 * @author Michael Zhou
 * @version $Id: CompositeExpressionFactory.java 685 2004-03-14 06:52:58Z baobao $
 */
...
public class CompositeExpressionFactory implements ExpressionFactory {
     ...
     * <ul>
     * <li>
     * 如果表达式中不包含“<code>${...}</code>”，则创建<code>ConstantExpression</code>。
     * </li>
     * <li>
     * 如果表达式以“<code>${</code>”开始，并以“<code>}</code>”结尾，则调用指定的<code>ExpressionFactory</code>来创建非组合表达式。
     * </li>
     * <li>
     * 如果表达式包含“<code>${...}</code>”，但在此之外还有别的字符，则创建<code>CompositeExpression</code>。
     * </li>
     * </ul>
     * 
     *
	...
	public Expression createExpression(String expr) throws ExpressionParseException {
        ...
        if (endIndex < 0) {
            throw new ExpressionParseException("Missing '}' character at the end of expression: "
                + expr);
        }
                                    ...
                                    while (i < length) {
...

                                        {
                                            switch (ch) {
                                                ...
                                                case ('}'): {
                                                    expressions.add(factory.createExpression(
                                                            exprBuff.toString()));
